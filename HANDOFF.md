...
My starting state — everything is installed and ready:
- Claude Code is installed (you're running it now)
- The repo is cloned and you're inside it
- Google Cloud account with billing enabled
- Homebrew installed
- gcloud CLI installed (you may still need to run `gcloud auth login` and `gcloud config set project ...`)
- Resend account created with an API key in hand

Please do this in order:
1. Read HANDOFF.md and CLAUDE.md, then give me a plain-language summary of what Periwink is and where things stand.
2. Walk me through Section 4 of HANDOFF.md — provision MY OWN Google Cloud project using infra/setup-gcp.sh. Stop before any step that costs money or creates a permanent resource and explain what's about to happen. When the script asks for a Resend key, ask me for it.
